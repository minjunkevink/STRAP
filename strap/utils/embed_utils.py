...
def get_dataset_info(file: h5py.File, output_file: tp.Union[None, h5py.File], model_keys, image_keys, key_0="demo_1"):
    ...
    for model_key in model_keys:
        ...
        for image_key in image_keys:
            if (grp.get(key_0) is not None) and (grp[key_0].get(model_key) is not None) and (grp[key_0][model_key].get(image_key) is not None):
...
def get_all_datasets_info(datasets_paths, model_keys, img_keys ):
    ...
    for dataset_path in datasets_paths:
        ...
        with h5py.File(original_file_path, "r", swmr=True) as original_file:
            if not is_file:
                dataset_info[original_file_path] = get_dataset_info(original_file, None, model_keys, img_keys)
            else:
                with h5py.File(dataset_out_file_path, "r", swmr=True) as save_file:
                    dataset_info[original_file_path] = get_dataset_info(original_file, save_file, model_keys, img_keys)
